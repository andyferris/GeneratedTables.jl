...
function printtable(io::IO, header_str::Vector{UTF8String}, data_str::Vector{Vector{UTF8String}}, row_header_str::UTF8String = UTF8String(""), row_str::Vector{UTF8String} = (length(data_str) > 0 ? fill(UTF8String(""), length(data_str[1])) : Vector{UTF8String}(""));
...
function Base.show(io::IO,table::Table)
    ...
    for c = 1:ncols
        width_suggestion = 20
        if eltype(table.(c)) <: Union{Bool,Nullable{Bool},Float64,Float32} && length(header_str[c]) < width_suggestion
            width_suggestion = length(header_str[c])
        end

        data_str[c] = make_strings(table.(c); maxl = maxl, width_suggestion=width_suggestion)
    end
...
function Base.showall(io::IO,table::Table)
    ...
    for c = 1:ncols
        ...
        data_str[c] = make_strings(table.(c); maxl = nrows, width_suggestion=width_suggestion)
    end
...
function Base.show(io::IO, col::Column)
    ...
    # header....
    header_str = [UTF8String(string(colname(col)))]

    data_str = Vector{UTF8String}[make_strings(col.(1); maxl = maxl, width_suggestion=60),]
...
function Base.showall(io::IO, col::Column)
    ...
    data_str = Vector{UTF8String}[make_strings(col.(1); maxl = nrows, width_suggestion=60),]
...
function Base.show(io::IO,row::Row)
    ...
    for c = 1:ncols
        width_suggestion = 20
        if eltype(row.(c)) <: Union{Bool,Nullable{Bool},Float64,Float32} && length(header_str[c]) < width_suggestion
            width_suggestion = length(header_str[c])
        end

        data_str[c] = make_strings([row.(c)]; width_suggestion=width_suggestion)
    end
...
function Base.show(io::IO, cell::Cell)
    ...
    # header....
    header_str = [UTF8String(string(colname(cell)))]

    data_str = Vector{UTF8String}[make_strings([cell.(1)]; width_suggestion=60),]
